...
def get_user_input(name, weeks, pay):
    allInput = name + "," + str(weeks) + "," + str(pay)
    return allInput

allThreeValues = get_user_input("Abc Efgh", 34, 345.93)

# let's see what all three input values look like when concatenated...
print("\n\n allInput = " + allThreeValues)

# so far so good, but how do we extract name, weeks and payrate?
# answer: we find the comma "," and slice the string! If you've programmed before,
# you may recognize this as a substring().

# find the first comma.
firstComma = allThreeValues.find(",")
print("\n first comma found at char number: " + str(firstComma))

# use this character position to slice our string and we have... name!
name = allThreeValues[0:firstComma]
print(name)
print("\n")

# find the second comma. We start looking at firstComma + 1.
secondComma = allThreeValues.find(",", firstComma+1)
print("\n second comma found at char number: " + str(secondComma))

# use this character position to slice our string and we have... weeks!
weeks = allThreeValues[secondComma+1]
print(weeks)
print("\n")

# We have only three values to parce, so this time we just need the position of the second comma (which we already have!)

# use this character position to slice our string and we have... weeks!
weekly_pay = allThreeValues[firstComma+1:secondComma]
print(weekly_pay)
print("\n")
